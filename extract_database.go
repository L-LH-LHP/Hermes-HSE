...
func main() {
	...
	// 创建文件ID->原始路径映射目录（供Web端根据fileID找到真实文档）
	if _, err := os.Stat("database_paths"); errors.Is(err, os.ErrNotExist) {
		err := os.Mkdir("database_paths", os.ModePerm)
		if err != nil {
			log.Println(err)
		}
	}
	...
	for _, folder := range senderFolders[0:] {
		...
		// 为当前用户创建文件ID->原始路径映射文件
		pathFile, perr := os.Create("database_paths/" + strconv.Itoa(userID) + ".txt")
		if perr != nil {
			fmt.Println("Failed to create path mapping file:", perr)
			return
		}
		...
		for _, subFolder := range senderSubFolders[1:] {
			...
			for _, file := range files {
				...
				// 记录 fileID 到原始文件路径的映射，供Web端根据fileID找到真实文档
				// 使用相对项目根目录的路径（例如 ./maildir/xxx）
				fmt.Fprintf(pathFile, "%d %s\n", fileID, filePath)
