...
// 重新加载时把旧的 PTkn/WTkn 移入此处，避免析构触发 PBC element_clear 导致 Bus error。用 list 避免 realloc 时移动元素（移动 PEKS_Token 会浅拷贝 element_t，析构时易导致 Segmentation fault）。
#ifdef WRITER_EFFICIENCY
static list<unordered_map<string, vector<Encrypted_Search_Token>>> wtkn_reload_graveyard;
#else
static list<unordered_map<string, vector<PEKS_Token>>> wtkn_reload_graveyard;
#endif
static list<unordered_map<string, vector<PEKS_Token>>> ptkn_reload_graveyard;

// Clear one writer's PTkn (element_clear each PEKS_Token)
static void clear_ptkn(int writer_id) {
    for (auto &it : PTkn[writer_id]) {
        for (auto &eptkn : it.second) {
            element_clear(eptkn.c1);
            element_clear(eptkn.c2);
            element_clear(eptkn.c3);
        }
    }
    PTkn[writer_id].clear();
}

#ifdef WRITER_EFFICIENCY
static void clear_wtkn(int writer_id) {
    for (auto &it : WTkn[writer_id]) {
        for (auto &ewtkn : it.second) {
            for (auto &kv : ewtkn.data) {
                element_clear(kv.second.c1);
                element_clear(kv.second.c2);
                element_clear(kv.second.c3);
            }
        }
    }
    WTkn[writer_id].clear();
}
#else
static void clear_wtkn(int writer_id) {
    for (auto &it : WTkn[writer_id]) {
...
// 前向声明，供 reload_from_database 调用
static void reload_from_database_impl(int n_writers);
static void reload_restore_from_graveyard(int n_writers);

// Reload index from database files (so search reflects updated database/*.txt)
void reload_from_database(int n_writers) {
    auto start = clock_start();
    cout << "Reloading index from database..." << endl;
    if (!EDTkn || !PTkn || !WTkn || n_writers <= 0) {
        cout << "Reload skipped: indices not initialized." << endl;
        return;
    }
    // 将旧 PTkn/WTkn 移入“墓地”并赋空 map，避免析构时触发 PBC element_clear 导致 Bus error。
    for (int writer_id = 0; writer_id < n_writers; ++writer_id) {
        EDTkn[writer_id].clear();
        ptkn_reload_graveyard.push_back(std::move(PTkn[writer_id]));
        PTkn[writer_id] = unordered_map<string, vector<PEKS_Token>>();
        wtkn_reload_graveyard.push_back(std::move(WTkn[writer_id]));
#ifdef WRITER_EFFICIENCY
        WTkn[writer_id] = unordered_map<string, vector<Encrypted_Search_Token>>();
#else
        WTkn[writer_id] = unordered_map<string, vector<PEKS_Token>>();
#endif
    }

    // 在 worker 线程中执行 reload，与 init 相同。主线程调用 HICKAE_Encrypt 会触发 libpbc curve_mul 崩溃，worker 线程则正常。
    try {
        auto fut = std::async(std::launch::async, [n_writers]() { reload_from_database_impl(n_writers); });
        fut.get();  // 等待完成
        cout << "Reload index done: " << time_from(start) << endl;
    } catch (const std::bad_alloc& e) {
        cout << "Reload failed: out of memory (too many writers/keywords). Restoring previous index." << endl;
        reload_restore_from_graveyard(n_writers);
    } catch (const std::exception& e) {
        cout << "Reload failed: " << e.what() << ". Restoring previous index." << endl;
        reload_restore_from_graveyard(n_writers);
    } catch (...) {
        cout << "Reload failed: unknown exception. Restoring previous index." << endl;
        reload_restore_from_graveyard(n_writers);
    }
}

// 失败时从 graveyard 恢复 PTkn/WTkn，避免索引被清空且进程继续运行
static void reload_restore_from_graveyard(int n_writers) {
    if ((int)ptkn_reload_graveyard.size() < n_writers || (int)wtkn_reload_graveyard.size() < n_writers) return;
    for (int writer_id = n_writers - 1; writer_id >= 0; --writer_id) {
        PTkn[writer_id] = std::move(ptkn_reload_graveyard.back());
        ptkn_reload_graveyard.pop_back();
        WTkn[writer_id] = std::move(wtkn_reload_graveyard.back());
        wtkn_reload_graveyard.pop_back();
    }
}

static void reload_from_database_impl(int n_writers) {
    unordered_map<string, uint64_t> *reload_state = new unordered_map<string, uint64_t>[n_writers];

#ifdef WRITER_EFFICIENCY
    vector<string> gamma_t;
    string padded_encoded_epoch = encoded_epoch;
    if (padded_encoded_epoch.size() < (size_t)DEPTH_EPOCH_TREE) {
        padded_encoded_epoch.insert(padded_encoded_epoch.end(), (size_t)DEPTH_EPOCH_TREE - padded_encoded_epoch.size(), '0');
    }
    ...
    for (int i = DEPTH_EPOCH_TREE - 1; i >= 0; --i) {
        if (i < (int)encoded_epoch.length() && (encoded_epoch.substr(0, i) + "1") == encoded_epoch.substr(0, i + 1)) {
            ...
            size_t pad = ((int)temp.size() < DEPTH_EPOCH_TREE) ? ((size_t)DEPTH_EPOCH_TREE - temp.size()) : 0;
            if (pad > 0) temp.insert(temp.end(), pad, '0');
            gamma_t.push_back(temp);
        }
    }
#endif

    // 16-byte aligned buffer for PRG seed (emp-tool block* requires alignment; unaligned access can cause Bus error)
    alignas(16) static char prg_seed_buf[32];
    static bool prg_seed_init = false;
    if (!prg_seed_init) {
        strncpy(prg_seed_buf, "generaterwritersecretkeys", sizeof(prg_seed_buf) - 1);
        prg_seed_buf[sizeof(prg_seed_buf) - 1] = '\0';
        prg_seed_init = true;
    }

    for (int writer_id = 0; writer_id < n_writers; ++writer_id) {
        alignas(16) unsigned char writer_secret_key[32];
        prg.reseed((block*)prg_seed_buf, writer_id + 1);
        ...
        while (getline(file, line)) {
            ...
            reload_state[writer_id][keyword] = 0;
            int file_id;
            unsigned char token[32], prev_token[32];
            memset(prev_token, 0, 32);
            while (iss >> file_id) {
                string seed = keyword + to_string(reload_state[writer_id][keyword]);
                ...
                if (md_ctx && EVP_DigestInit_ex(md_ctx, EVP_sha512(), NULL) == 1
                    && EVP_DigestUpdate(md_ctx, token, 16) == 1
                    && EVP_DigestFinal_ex(md_ctx, tmp, NULL) == 1) {
                ...
                SHA512_CTX sha_ctx;
                SHA512_Init(&sha_ctx);
                SHA512_Update(&sha_ctx, token, 16);
                SHA512_Final(tmp, &sha_ctx);
#endif
                memset(addr, 0, sizeof(addr));
                for (int j = 0; j < 10; ++j) sprintf(addr + j * 2, "%02x", tmp[j]);
                addr[20] = '\0';
                string addr_key(addr, 20);
                DSSE_Token value;
                ...
                for (int j = 0; j < 37; ++j) value[j] ^= tmp[j + 10];
                memcpy(EDTkn[writer_id][addr_key], value, sizeof(DSSE_Token));
                memcpy(prev_token, token, 32);
                reload_state[writer_id][keyword]++;
            }
            ...
            memset(addr, 0, sizeof(addr));
            for (int j = 0; j < 10; ++j) sprintf(addr + j * 2, "%02x", partition_tag[j]);
            string paddr(addr, addr + 20);
            string last_paddr = paddr;
#ifdef SEARCH_EFFICIENCY
            if (WTkn[writer_id][paddr].empty()) {
                bool exist = false;
                int num_partitions_local = NUM_PARTITIONS;
                for (int k = 1; k < RECURSIVE_LEVEL; ++k) {
                    ...
                    num_partitions_local /= PARTITION_SIZE;
                    hash_value = mm_hash((uint8_t*)&pid, sizeof(pid));
                    pid = ((hash_value[0] % num_partitions_local) << 2) | (RECURSIVE_LEVEL - k);
                    prf((unsigned char *)&pid, sizeof(pid), writer_secret_key, partition_tag);
                    memset(addr, 0, sizeof(addr));
                    for (int j = 0; j < 10; ++j) sprintf(addr + j * 2, "%02x", partition_tag[j]);
...
void init(int num_writers) {
    ...
    for(int i = DEPTH_EPOCH_TREE - 1; i >= 0; --i) {
        if((encoded_epoch.substr(0, i) + "1") == encoded_epoch.substr(0, i + 1)) {
            string temp = encoded_epoch.substr(0, i) + "2";
            size_t pad = (temp.size() < (size_t)DEPTH_EPOCH_TREE) ? ((size_t)DEPTH_EPOCH_TREE - temp.size()) : 0;
            if (pad > 0) temp.insert(temp.end(), pad, '0');
            gamma_t.push_back(temp);
        }
    }
...
void search(vector<int> &writer_subset, uint8_t *search_query) {
    ...
    for(int k = 0; k < RECURSIVE_LEVEL; ++k) {
        ...
        if (temp > MAX_MPZ_IMPORT_SIZE) {
            cout << "Invalid search query: mpz size " << temp << " exceeds max " << MAX_MPZ_IMPORT_SIZE << ", aborting." << endl;
            mpz_clear(cp[k].k1);
            for (int j = 0; j < k; ++j) { element_clear(cp[j].k2); element_clear(cp[j].k3); mpz_clear(cp[j].k1); }
            zmq::message_t err_reply(writer_subset.size() * sizeof(int));
            uint8_t *p = (uint8_t*)err_reply.data();
            for (size_t i = 0; i < writer_subset.size(); ++i) { int z = 0; memcpy(p, &z, sizeof(int)); p += sizeof(int); }
            socket_server->send(err_reply);
            return;
        }
    ...
    if (n <= 0 || n > 128) {
        cout << "Invalid search query: n=" << n << " out of range, aborting." << endl;
#ifdef SEARCH_EFFICIENCY
        for (int j = 0; j < RECURSIVE_LEVEL; ++j) { element_clear(cp[j].k2); element_clear(cp[j].k3); mpz_clear(cp[j].k1); }
#else
        element_clear(cp.k2); element_clear(cp.k3); mpz_clear(cp.k1);
#endif
        zmq::message_t err_reply(writer_subset.size() * sizeof(int));
        uint8_t *p = (uint8_t*)err_reply.data();
        for (size_t i = 0; i < writer_subset.size(); ++i) { int z = 0; memcpy(p, &z, sizeof(int)); p += sizeof(int); }
    ...
    for(int k = 0; k < n; ++k) {
        ...
        search_query += sizeof(size_t);
        if (temp > MAX_MPZ_IMPORT_SIZE) {
            cout << "Invalid search query: cw mpz size " << temp << " exceeds max " << MAX_MPZ_IMPORT_SIZE << ", aborting." << endl;
            for (int j = 0; j < k; ++j) { element_clear(cw[j].k2); element_clear(cw[j].k3); mpz_clear(cw[j].k1); }
            mpz_clear(cw[k].k1);
            delete[] cw;
#ifdef SEARCH_EFFICIENCY
            for (int j = 0; j < RECURSIVE_LEVEL; ++j) { element_clear(cp[j].k2); element_clear(cp[j].k3); mpz_clear(cp[j].k1); }
        ...
        if(found) {
            ...
            while(1) {
                ...
                if (EDTkn[writer_id].find(addr) != EDTkn[writer_id].end())
                {
                    ...
                    if (value[0] == 0) {
                        // 已删除节点（tombstone）：不加入结果，沿 next_token 继续
                        memcpy(search_token, (uint8_t*)value + 5, 32);
                    } else {
                        memcpy(&output[writer_id][count+1], value + 1, sizeof(int));
                        count++;
...
*/
            ...
            PTkn[writer_id][paddr].push_back(eptkn);
#ifdef SEARCH_EFFICIENCY
            // 搜索时先从 PTkn[writer_id][""] 解密得到 paddr，新增分区需在此登记才能被搜到
            PTkn[writer_id][""].push_back(eptkn);
#endif
...
// Delete: 删除指定 EDB 节点（op=删除，Updt 算法），使用 tombstone 占位以保持链连续性
// 消息格式: writer_id(4) num_deletes(4) [ addr_del(20) value_del(37) ]*
void update_delete(uint8_t *del_query) {
    auto start = clock_start();
    int writer_id;
    memcpy(&writer_id, del_query, 4);
    del_query += 4;
    int num_deletes;
    memcpy(&num_deletes, del_query, 4);
    del_query += 4;
    char addr_del[21];
    addr_del[20] = '\0';
    for (int i = 0; i < num_deletes; ++i) {
        memcpy(addr_del, del_query, 20);
        del_query += 20;
        uint8_t *value_del = del_query;
        del_query += 37;
        
        // 无论是在链中还是链首，统一用 tombstone 覆盖原节点（op=0, prev_token），检索时跳过并继续
        memcpy(EDTkn[writer_id][string(addr_del, 20)], value_del, sizeof(DSSE_Token));
    }
    char *ack_msg = "ACK";
    zmq::message_t ack(strlen(ack_msg) + 1);
    memcpy(ack.data(), ack_msg, strlen(ack_msg) + 1);
    socket_server->send(ack);
    cout << "Server delete latency: " << time_from(start) << endl;
}
    ...
    while(1) {
        ...
        switch(query_data[0]) {
            ...
            case 'S':  
                ...
                for(int i = 0; i < writer_subset_size; ++i) {
                    int w_id;
                    memcpy(&w_id, query_data, sizeof(int));
                    writer_subset.push_back(w_id);
                    query_data += sizeof(int);
                }
            ...
            case 'U':
                update(query_data + 1);
                break;
            // Delete (op=删除): 删除旧关键字-文件映射并重链，无需清空整写者
            case 'D':
                if (query.size() >= 1u + 8u) update_delete(query_data + 1);
                break;
            // Clear one writer's index (EDTkn/PTkn/WTkn) so client can re-push full database for that writer
            case 'C': {
                if (query.size() < 1 + 4u) break;
                int cw;
                memcpy(&cw, query_data + 1, 4);
                if (cw >= 0 && cw < num_writers) {
                    EDTkn[cw].clear();
                    clear_ptkn(cw);
                    clear_wtkn(cw);
                    memcpy(reply.data(), "OK", 3);
                    static_cast<char*>(reply.data())[3] = '\0';
                }
            ...
            // Reload index from database files (so search reflects updated database/*.txt)
            case 'I':
#if RELOAD_DISABLE_IN_PROCESS
                memcpy(reply.data(), "RST", 4);  // 不执行 reload，提示客户端需重启 server
#else
                reload_from_database(num_writers);
                memcpy(reply.data(), "OK", 3);
                static_cast<char*>(reply.data())[3] = '\0';
#endif
                socket_server->send(reply, zmq::send_flags::none);
