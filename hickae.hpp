...
// Reload 时使用全局 random_state，避免主线程 thread_local 首次初始化与 PBC curve_mul 的潜在冲突
extern bool g_hickae_in_reload;
...
// 每线程独立 random_state，避免多线程共享全局 random_state 导致状态破坏；同时保持 init 多线程并行。
// Reload 时主线程使用全局 random_state，避免 thread_local 首次初始化与 PBC curve_mul 的潜在冲突。
static gmp_randstate_t& hickae_encrypt_rand() {
    if (g_hickae_in_reload) return random_state;
    thread_local gmp_randstate_t tl_rand;
    thread_local bool tl_init = false;
    if (!tl_init) {
        gmp_randinit_mt(tl_rand);
        unsigned long seed = (unsigned long)std::hash<std::thread::id>()(std::this_thread::get_id()) ^ (unsigned long)time(NULL);
        gmp_randseed_ui(tl_rand, seed ? seed : 1);
        tl_init = true;
    }
    return tl_rand;
}
...
void HICKAE_Encrypt(int wid, char *id, unsigned char *m, PEKS_Token *c) {
    ...
    mpz_t r;
    mpz_init(r);
    mpz_urandomb(r, hickae_encrypt_rand(), 224);
    // printf("r = ");
